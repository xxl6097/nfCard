...
public final class NfcPage implements ReaderListener {
	...
	public static CharSequence getContent(Activity activity, Intent intent) {
		...
		return new SpanFormatter(AboutPage.getActionHandler(activity)).toSpanned(info);
	}
	...
	private Intent buildResult(Card card) {
		...
		if (card != null && !card.hasReadingException()) {
			if (card.isUnknownCard()) {
				String cardInfo = ThisApplication .getStringResource(R.string.info_nfc_unknown);
				String tagId = card.getStringProperty(SPEC.PROP.ID);
				System.out.println("card id:"+tagId);
				cardInfo = String.format(cardInfo, tagId);
				ret.putExtra(RET, cardInfo);
